...
class CreateGateAccessScreen extends StatefulWidget {
  const CreateGateAccessScreen({super.key});

  @override
  State<CreateGateAccessScreen> createState() => _CreateGateAccessScreenState();
}

class _CreateGateAccessScreenState extends State<CreateGateAccessScreen> {
  ...
  // State
  String _visitorType = 'employee';
  String _accessType = 'both';
  String? _vehicleType;
  String? _cccdPhotoUrl;

  // Time state
  bool _isMultipleDays = false; // Đăng ký nhiều ngày
  ...
  void _prefillUserData() {
    ...
    if (user != null) {
      ...
      _addressOrCompanyController.text = user.company ?? '';
      _visitorType = user.isContractor ? 'visitor' : 'employee';
    }
  }
  ...
  Future<void> _selectTime(bool isFrom) async {
    final time = await showTimePicker(
      context: context,
      initialTime: isFrom
          ? (_expectedTimeFrom ?? const TimeOfDay(hour: 8, minute: 0))
  ...
  Future<void> _handleSubmit() async {
    ...
    final user = context.read<AuthProvider>().user;
    if (user == null) {
      Helpers.showErrorSnackBar(context, 'Không tìm thấy thông tin người dùng');
      return;
    }
    ...
    final success = await provider.createRegistration(
      user: user,
    ...
    if (success && mounted) {
      Helpers.showSuccessSnackBar(
        context,
        'Đã gửi yêu cầu đăng ký. Vui lòng chờ duyệt.',
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Đăng ký ra/vào cổng'),
      ),
      body: Form(
        key: _formKey,
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'Sau khi gửi, đăng ký sẽ chờ Admin duyệt. Bạn sẽ nhận thông báo khi được duyệt.',
                        style: TextStyle(
